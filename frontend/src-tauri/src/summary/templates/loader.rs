...
/// - macOS: ~/Library/Application Support/Wicflow Meet/templates/
/// - Windows: %APPDATA%\Wicflow Meet\templates\
/// - Linux: ~/.config/Wicflow Meet/templates/
fn get_custom_templates_dir() -> Option<PathBuf> {
    let mut path = dirs::data_dir()?;
    path.push("Wicflow Meet");
    path.push("templates");
    Some(path)
}
