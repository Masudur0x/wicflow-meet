...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\wicflow-meet-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("wicflow-meet-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("wicflow-meet-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/wicflow-meet-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("wicflow-meet-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/wicflow-meet-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("wicflow-meet-recordings")
    }
